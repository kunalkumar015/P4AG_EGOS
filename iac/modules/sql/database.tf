resource "azurerm_mssql_server" "dbserver" {
  name                         = "dbsrv2-wwe-shared-${local.environment_sanitized}-${local.region_sanitized}"
...
resource "azurerm_mssql_database" "sqldb" {
  ...
  name           = each.value //"sqldb-wwe-${local.app_type}-${local.environment_sanitized}-${local.region_sanitized}"
